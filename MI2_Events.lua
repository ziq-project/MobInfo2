...
local function MI2_MobInfo_Initialize()
	MI2_CharTable = MI2_CharTable or {}

	-- Guard against corrupted/nil SavedVariables entries. If the WTF file
	-- somehow saved these as "= nil" (e.g. after a crash or an oversized
	-- database write), the {} defaults set at the top of MobInfo2.lua get
	-- overwritten with nil here at VARIABLES_LOADED, causing "attempt to
	-- index global (a nil value)" errors later in loot/zone/tooltip code.
	...
	if not MI2_CharTable[charName] then
		MI2_CharTable.charCount = (MI2_CharTable.charCount or 0) + 1
...
--	GameTooltip:SetScript( "OnShow", MI2_GameTooltip_OnShow )
	...
	for idx,val in MI2_ZoneTable or {} do
		if idx ~= "cnt" then
			MI2_Zones[0][val] = idx
			MI2_Zones[1][val] = idx
			MI2_Zones[2][val] = idx
